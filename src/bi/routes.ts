...
export async function registerBiRoutes(app: FastifyInstance, config: AppConfig): Promise<void> {
  const db = getDb(config);

  async function q<T extends RowDataPacket>(sql: string, params: (string | number)[] = []): Promise<T[]> {
    const [rows] = await db.execute<T[]>(sql, params);
    return rows;
  }

  // Parse from/to strings, fall back to `days` offset from today
  function parsePeriod(qs: Record<string, string>): [string, string] {
  ...
  app.get('/api/bi/metrics', async (req, reply) => {
    ...
    try {
      const [
        ...
        uniqueUsersPrev,
        authenticatedUsers,
        authenticatedUsersPrev,
        newUsers,        // first message in period
        ...
        // ── Sessões ──────────────────────────────────────────────────────────
        q<RowDataPacket>(
          `SELECT COUNT(DISTINCT phone) AS total FROM messages
        ...
        q<RowDataPacket>(
          `SELECT ROUND(AVG(cnt)) AS avg_msgs FROM (
        ...
        // ── Funil ─────────────────────────────────────────────────────────────
        q<RowDataPacket>(
          `SELECT COUNT(DISTINCT phone) AS total FROM otp_codes
           WHERE DATE(created_at) BETWEEN ? AND ?`,
          [from, to]
        ),
        q<RowDataPacket>(
          `SELECT COUNT(DISTINCT phone) AS total FROM otp_codes
           WHERE used_at IS NOT NULL AND DATE(created_at) BETWEEN ? AND ?`,
          [from, to]
        ),
        ...
        q<RowDataPacket>(
          `SELECT COUNT(*) AS total FROM human_tickets
           WHERE DATE(created_at) BETWEEN ? AND ?`,
          [from, to]
        ),
        // Menu prev period
        ...
        // ── Tickets ───────────────────────────────────────────────────────────
        q<RowDataPacket>(
          `SELECT status, COUNT(*) AS count FROM human_tickets GROUP BY status`
        ),
        q<RowDataPacket>(
          `SELECT COUNT(*) AS total FROM human_tickets
           WHERE DATE(created_at) BETWEEN ? AND ?`,
        ...
        q<RowDataPacket>(
          `SELECT ROUND(AVG(TIMESTAMPDIFF(MINUTE, created_at, updated_at))) AS avg_min
           FROM human_tickets
           WHERE status = 'finalizado' AND DATE(created_at) BETWEEN ? AND ?`,
          [from, to]
        ),
        q<RowDataPacket>(
          `SELECT ROUND(AVG(TIMESTAMPDIFF(MINUTE, created_at, updated_at))) AS avg_min
        ...
        q<RowDataPacket>(
          `SELECT assigned_attendant, COUNT(*) AS total,
        ...
        // ── OTP ───────────────────────────────────────────────────────────────
        q<RowDataPacket>(
          `SELECT COUNT(*) AS total FROM otp_codes
           WHERE DATE(created_at) BETWEEN ? AND ?`,
          [from, to]
        ),
        q<RowDataPacket>(
        ...
        q<RowDataPacket>(
          `SELECT COUNT(*) AS total FROM otp_codes
           WHERE attempts > 0 AND DATE(created_at) BETWEEN ? AND ?`,
          [from, to]
        ),
        ...
        q<RowDataPacket>(
          `SELECT status, COUNT(*) AS count, ROUND(SUM(valor), 2) AS valor
           FROM faturas
        ...
        q<RowDataPacket>(
          `SELECT ROUND(SUM(valor), 2) AS total, COUNT(*) AS count
           FROM faturas
           WHERE DATE(vencimento) BETWEEN ? AND ?`,
          [from, to]
        ).catch(() => [] as RowDataPacket[]),
      ...
      return {
        ...
        overview: {
          ...
          otpSuccessRateDelta: delta(
            otpTot > 0 ? Math.round((otpSuc / otpTot) * 100) : 0,
            otpTotPrev > 0 ? Math.round((otpSucPrev / otpTotPrev) * 100) : 0
          ),
  ...
  app.get('/api/bi/debug', async (_, reply) => {
    try {
      const tables = await q<RowDataPacket>(`SHOW TABLES`);
      const key = Object.keys(tables[0] ?? {})[0];
      const names = tables.map(r => String(r[key]));

      const cols: Record<string, string[]> = {};
      for (const t of names) {
        const c = await q<RowDataPacket>(`SHOW COLUMNS FROM \`${t}\``);
  ...
  app.get('/api/bi/debug/audit', async (_, reply) => {
    try {
      const actions = await q<RowDataPacket>(
        `SELECT action, COUNT(*) AS total FROM audit_logs GROUP BY action ORDER BY total DESC`
      );
      const sample = await q<RowDataPacket>(
        `SELECT action, whatsapp_phone, cpf, payload, created_at FROM audit_logs ORDER BY created_at DESC LIMIT 10`
      );
  ...
  app.get('/api/bi/debug/faturas', async (_, reply) => {
    try {
      const [count]   = await q<RowDataPacket>(`SELECT COUNT(*) AS total FROM faturas`);
      const [ranges]  = await q<RowDataPacket>(`
        SELECT
          COUNT(*) AS total,
        ...
        FROM faturas
      `);
      const byStatus  = await q<RowDataPacket>(`SELECT status, COUNT(*) AS cnt FROM faturas GROUP BY status`);
      const sample    = await q<RowDataPacket>(`SELECT id, identificador, referencia, status, valor, vencimento, created_at FROM faturas ORDER BY vencimento DESC LIMIT 5`);
  ...
  app.get('/api/bi/detail/sessions', async (req, reply) => {
    ...
    try {
      ...
      const [countRow] = await q<RowDataPacket>(
        `SELECT COUNT(DISTINCT m.phone) AS total FROM messages m ${baseWhere}`,
        params
      );
  ...
  app.get('/api/bi/detail/tickets', async (req, reply) => {
    ...
    try {
      const params: (string | number)[] = [from, to];
      let extra = '';
      if (status)    { extra += ` AND status = ?`;                params.push(status); }
      if (attendant) { extra += ` AND assigned_attendant = ?`;    params.push(attendant); }
  ...
  app.get('/api/bi/detail/otp', async (req, reply) => {
    ...
    try {
      ...
      if (filter === 'used')    extra = ` AND used_at IS NOT NULL`;
      if (filter === 'expired') extra = ` AND used_at IS NULL AND expires_at < NOW()`;
      if (filter === 'errors')  extra = ` AND attempts > 0`;
  ...
  app.get('/api/bi/detail/financeiro', async (req, reply) => {
    ...
    try {
      ...
      const [countRow] = await q<RowDataPacket>(
        `SELECT COUNT(*) AS total FROM faturas
         WHERE DATE(vencimento) BETWEEN ? AND ? ${extra}`,
        params
      ...
      return { total: Number((countRow as RowDataPacket[])[0]?.total ?? 0), from, to, rows };
    } catch (err) {
      return reply.code(500).send({ error: (err as Error).message });
    }
  });
