...
import { linkGetDadosCadastrais, isLinkApiConfigured, type LinkDadosCadastrais } from './linkApi.js';

export type DadosCadastraisLigacao = {
 ...
 */
function linkDadosToDadosCadastrais(dados: LinkDadosCadastrais): DadosCadastraisLigacao {
 ...
 */
export async function fetchDadosCadastraisByImovelId(
  config: AppConfig,
  imovelId: number
): Promise<DadosCadastraisLigacao | null> {
...
function buildCadastroUrl(config: AppConfig, cpf: string, ligacaoId: string): string {
 ...
 */
export async function fetchDadosCadastraisByLigacao(
  config: AppConfig,
  params: { cpf?: string; ligacaoId: string; imovelId?: number }
): Promise<DadosCadastraisLigacao | null> {
  ...
  const numeroLigacao =
    fonte.numeroLigacao || fonte.numero_ligacao || fonte.ligacaoId || fonte.idLigacao || fonte.id_ligacao;
  const nomeTitular = fonte.nomeTitular || fonte.titular || fonte.nome || fonte.nome_cliente;
  const numeroHidrometro =
    fonte.numeroHidrometro || fonte.hidrometro || fonte.num_hidrometro || fonte.medidor;
  const situacaoAbastecimento =
    fonte.situacaoAbastecimento || fonte.situacao || fonte.statusAbastecimento || fonte.status_abastecimento;

  const servicosContratadosRaw =
    fonte.servicosContratados || fonte.servicos || fonte.servicos_contratados || fonte.tipos_servico;

  let servicosContratados: string[] | undefined;
  if (Array.isArray(servicosContratadosRaw)) {
    servicosContratados = servicosContratadosRaw.map((s: any) => String(s));
  ...
  const enderecoImovel =
    fonte.enderecoImovel || fonte.endereco_imovel || fonte.endereco || fonte.enderecoInstalacao;
  const enderecoCorrespondencia =
    fonte.enderecoCorrespondencia || fonte.endereco_correspondencia || fonte.enderecoCobranca;
  const categoria = fonte.categoria || fonte.classe || fonte.tipoLigacao || fonte.tipo_ligacao;
  const dataAtivacao =
    fonte.dataAtivacao || fonte.data_ativacao || fonte.ativacaoData || fonte.dataLigacao || fonte.data_ligacao;
