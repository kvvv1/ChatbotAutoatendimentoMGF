...
import { linkGetUltimasLeituras, isLinkApiConfigured, type LinkLeitura } from './linkApi.js';

export type ConsumoLeitura = {
  referencia: string; // ex.: "02/2026"
  dataLeitura?: string;
  leitura?: number;
  consumoReal?: number;
 ...
 */
function linkLeituraToConsumo(item: LinkLeitura): ConsumoLeitura {
  return {
 ...
 */
export async function fetchConsumoByImovelId(
  config: AppConfig,
  imovelId: number
): Promise<ConsumoLeitura[]> {
...
function buildConsumoUrl(config: AppConfig, cpf: string, ligacaoId: string): string {
 ...
 */
export async function fetchConsumoByLigacao(
  config: AppConfig,
  ...
  const lista: ConsumoLeitura[] = rawList.map((item, index) => {
    const referencia =
      (item && (item.referencia || item.mesAno || item.mes_ano || item.mesAnoReferencia)) ||
      `Período ${index + 1}`;

    const dataLeitura =
      (item && (item.dataLeitura || item.data_leitura || item.leituraData || item.leitura_data)) ||
      undefined;

    const consumoRealRaw =
      item && (item.consumoReal || item.consumo_real || item.consumoMedido || item.consumo_medido);
    const consumoFaturadoRaw =
      item && (item.consumoFaturado || item.consumo_faturado || item.consumo || item.kwh_faturado);
    const mediaRaw =
      item && (item.mediaConsumo || item.media_consumo || item.consumoMedio || item.consumo_medio);

    const consumoRealKwh =
      consumoRealRaw !== undefined && consumoRealRaw !== null ? Number(consumoRealRaw) || undefined : undefined;
    const consumoFaturadoKwh =
      consumoFaturadoRaw !== undefined && consumoFaturadoRaw !== null
        ? Number(consumoFaturadoRaw) || undefined
        : undefined;
    const mediaConsumoKwh =
      mediaRaw !== undefined && mediaRaw !== null ? Number(mediaRaw) || undefined : undefined;
