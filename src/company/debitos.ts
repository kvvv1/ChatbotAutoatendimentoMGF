...
import { linkGetDebitos, isLinkApiConfigured, type LinkDebito } from './linkApi.js';
 ...
 */
function formatDataVencimento(dataVencimento: string): string {
  try {
 ...
 */
export async function fetchDebitosByImovelId(
  config: AppConfig,
  imovelId: number
): Promise<Debito[]> {
...
function buildDebitosUrl(config: AppConfig, cpf: string, ligacaoId: string): string {
 ...
 */
export async function fetchDebitosByLigacao(
  config: AppConfig,
  params: { cpf?: string; ligacaoId: string; imovelId?: number | string }
): Promise<Debito[]> {
  const { cpf, ligacaoId } = params;
  
  // Converte imovelId para número se necessário
  let imovelIdNum: number | undefined;
  if (params.imovelId !== undefined && params.imovelId !== null) {
    ...
    if (isNaN(imovelIdNum)) imovelIdNum = undefined;
  }
  ...
  const debitos: Debito[] = rawList.map((item, index) => {
    const idFatura =
      (item && (item.idFatura || item.faturaId || item.numeroFatura || item.nossoNumero || item.id)) ||
      String(index + 1);

    const mesAnoReferencia =
      (item && (item.mesAnoReferencia || item.referencia || item.mesAno || item.mes_ano)) ||
      '';

    const dataVencimento =
      (item && (item.dataVencimento || item.vencimento || item.data_vencimento)) ||
      '';

    const valorRaw =
      (item && (item.valor || item.valorFatura || item.valor_total || item.total)) ||
      0;
    const valor = Number(valorRaw) || 0;

    const emDebitoAutomatico = Boolean(
      item && (item.emDebitoAutomatico || item.debitoAutomatico || item.debito_automatico || item.automaticDebit)
    );

    const linhaDigitavel =
      (item && (item.linhaDigitavel || item.linha_digitavel || item.codigoBarras || item.codigo_barras)) ||
      '';

    const status: string | undefined =
      (item && (item.status || item.situacao || item.situacao_fatura)) ||
      undefined;

    const urlFatura: string | undefined =
      (item && (item.urlFatura || item.url_fatura || item.urlBoleto || item.url_boleto || item.pdfUrl || item.pdf_url)) ||
      undefined;
