...
export interface DadosAutarquia {
  nome: string;
  nomeCompleto: string;
  cnpj: string;
...
let cachedAutarquia: DadosAutarquia | null = null;
let cacheTimestamp: number = 0;
const CACHE_TTL_MS = 60 * 60 * 1000; // 1 hora

export async function fetchDadosAutarquia(config: AppConfig): Promise<DadosAutarquia> {
  // Verifica cache
  const now = Date.now();
  ...
  const autarquia: DadosAutarquia = {
    nome: data.Nome || 'SAAE',
 ...
 */
export function clearAutarquiaCache(): void {
