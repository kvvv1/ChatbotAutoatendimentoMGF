...
export type WhatsappCheck = CheckResult & {
  connected?: boolean;
...
async function checkZapi(instance: Instance): Promise<WhatsappCheck> {
...
async function checkGestcom(instance: Instance): Promise<CheckResult> {
  ...
  if (!url) return { ok: false, detail: 'Não configurado' };
...
function checkSmtp(host: string, port: number): Promise<CheckResult> {
  if (!host) return Promise.resolve({ ok: false, detail: 'Não configurado' });
...
export async function checkInstance(instance: Instance): Promise<InstanceHealth> {
