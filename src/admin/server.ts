...
async function bootstrap() {
  ...
  app.get<{ Params: { slug: string } }>('/admin/api/status/:slug', async (req, reply) => {
    const instance = getInstance(req.params.slug);
    if (!instance) return reply.code(404).send({ error: 'Entidade não encontrada' });
    return checkInstance(instance);
  });

  // ── QR Code ───────────────────────────────────────────────────
  app.get<{ Params: { slug: string } }>('/admin/api/qrcode/:slug', async (req, reply) => {
    const instance = getInstance(req.params.slug);
    if (!instance) return reply.code(404).send({ error: 'Entidade não encontrada' });
  ...
  // ── ENV ───────────────────────────────────────────────────────
  app.get<{ Params: { slug: string } }>('/admin/api/env/:slug', async (req, reply) => {
    const instance = getInstance(req.params.slug);
    if (!instance) return reply.code(404).send({ error: 'Entidade não encontrada' });
  ...
  // ── Auditoria ─────────────────────────────────────────────────
  app.get<{ Querystring: Record<string, string> }>('/admin/api/audit', async (req, reply) => {
    ...
    if (!instance) return reply.code(404).send({ error: 'Entidade não encontrada' });
    if (!instance.dbHost) return reply.code(422).send({ error: 'DB não configurado para esta entidade' });
