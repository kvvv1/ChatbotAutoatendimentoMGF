 ...
 */
export function createApiAuthHook(secret, pathPrefixes) {
    const prefixes = Array.isArray(pathPrefixes) ? pathPrefixes : [pathPrefixes];
    return async (request, reply) => {
        if (!secret)
            return;
        if (!prefixes.some(p => request.url.startsWith(p)))
            return;
        const header = request.headers['authorization'];
        const headerToken = typeof header === 'string' ? header.replace(/^Bearer\s+/i, '').trim() : '';
        const queryToken = typeof request.query?.key === 'string' ? request.query.key : '';
