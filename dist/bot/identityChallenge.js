...
function extractBairro(endereco) {
    ...
    if (parts.length >= 2) {
        ...
        if (bairro && !bairro.includes('/') && bairro.length > 1)
            return bairro;
    }
    return null;
}
...
// ─── Builders por campo ──────────────────────────────────────────────────────
function buildCpfChallenge(cpf) {
...
function buildPhoneChallenge(phone) {
...
function buildEmailChallenge(email) {
    ...
    const decoyPrefixos = shuffle(PREFIXOS_EMAIL.filter(p => p !== local.substring(0, 3)));
...
function buildBairroChallenge(bairro) {
    const decoys = shuffle(BAIRROS_DECOY.filter(b => b.toUpperCase() !== bairro.toUpperCase()));
    const options = shuffle([
...
function buildNomeChallenge(nomeCompleto) {
    // Usa apenas o primeiro nome para o botão (mais curto, mais legível)
    const primeiroNome = capitalize(nomeCompleto.trim().split(/\s+/)[0]);
    const decoys = shuffle(NOMES_DECOY.filter(n => n.toLowerCase() !== primeiroNome.toLowerCase()));
    const options = shuffle([
 ...
 */
export function buildChallenges(dados) {
    ...
    if (tel) {
        const digits = String(tel).replace(/\D/g, '');
        if (digits.length >= 4)
            return { primary: buildPhoneChallenge(digits) };
    }
    if (dados.Email && typeof dados.Email === 'string' && dados.Email.includes('@')) {
        const [local] = dados.Email.split('@');
        if (local.length >= 2)
            return { primary: buildEmailChallenge(dados.Email) };
    }
    if (dados.Endereco) {
        const bairro = extractBairro(dados.Endereco);
        if (bairro)
            return { primary: buildBairroChallenge(bairro) };
    }
    if (dados.Nome && dados.Nome.trim().length >= 2) {
