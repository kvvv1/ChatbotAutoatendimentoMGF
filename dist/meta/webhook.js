 ...
 */
export async function registerMetaRoutes(rootApp, config) {
                            ...
                            try {
                                if (typeof out === 'string') {
                                    ...
                                    try {
                                        ...
                                        const buttonsText = (o.buttons || []).map((b, idx) => `${idx + 1} - ${b.text}`).join('\n');
                                    ...
                                    const o = out;
                                    const options = []
                                        .concat(...(o.sections || []).map((s) => s.rows || []))
                                ...
                                try {
                                    await logMessage(config, { phone, direction: 'out', content: typeof out === 'string' ? out : JSON.stringify(out) });
                                }
                                catch { }
                            }
                            catch (err) {
